...
export class WelcomeComponent implements OnDestroy {
  ...
  private entranceAnimation?: gsap.Context;
  ...
  private mountWelcomeAnimations(): void {
    ...
    this.entranceAnimation = gsap.context(() => {
      if (runEntrance) {
        ...
        timeline
          ...
          .from('.welcome__badge', { y: 24, opacity: 0, duration: 0.55 }, '-=0.4')
          .from(
            '.welcome__title-word',
            { y: 48, opacity: 0, rotateX: -12, duration: 0.65, stagger: 0.08 },
            '-=0.35',
          )
          .from('.welcome__lead', { y: 20, opacity: 0, duration: 0.5 }, '-=0.25')
          .from(
